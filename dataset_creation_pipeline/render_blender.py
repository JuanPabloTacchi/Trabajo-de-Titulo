...
#Aqui es donde se reescala

#for obj in bpy.context.selected_objects:
dimensions = obj.dimensions
max_dimension = max(dimensions)
if max_dimension > 0:
    scale_factor = 0.5 / max_dimension
    obj.scale = (scale_factor, scale_factor, scale_factor)
    bpy.context.view_layer.objects.active = obj
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
    print(f"El objeto '{obj.name}' ha sido normalizado al rango [-1, 1].")
else:
    print(f"El objeto '{obj.name}' tiene dimensiones cero y no puede ser normalizado.")
...
bg_nodes["Background"].inputs[0].default_value = (1, 1, 1, 1)  # RGB + Alpha (blanco)
bg_nodes["Background"].inputs[1].default_value = 1.0
...
model_identifier = os.path.split(fp)[-1]
for i in range(0, args.views):
    print("Rotation {}, {}".format((stepsize * i), math.radians(stepsize * i)))

    render_file_path = fp + "/" + model_identifier + '_r_{0:03d}'.format(int(i * stepsize))
