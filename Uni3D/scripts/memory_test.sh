#Testing of the model memory use in gpu and inference time
#!/bin/bash
WORLD_SIZE=1
...
ckpt_path="/path/to/checkpoint/model_g.pt" \
#only use 1 gpu and batch size of 1
CUDA_VISIBLE_DEVICES=1 torchrun --nnodes=$WORLD_SIZE \
    --nproc-per-node=1 \
    main.py \
    --model $model \
    --Experiment_detail "memory test"\
    ...
    --optimizer="adamw" \
    --zero-stage=1 \
    --validate_dataset_name ShapeTest \
