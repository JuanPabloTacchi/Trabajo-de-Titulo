...
class ShapeTrain(data.Dataset):
    def __init__(self, config):
        ...
        self.train_transform = config.train_transform
        #self.id_map_addr = config.DATA_PATH
        ...
        print_log(f'[DATASET] sample out {self.sample_points_num} points', logger='TrainShape')
        ...
        for dir in os.listdir(self.data_root):
            #if dir not in selected_list:
            #    continue
            pc_extension = [".ply"]
            img_extension = [".png", ".jpg"]
            txt_extension = [".txt"]
            numpy_extension = [".npy"]
            object_file = os.path.join(self.data_root, dir) 
            pc_dir = os.path.join(object_file, dir + ".ply")
            for image_file in os.listdir(object_file):
                #in case of image_file = the ply object
                pc_name, extension = os.path.splitext(image_file)
                if extension in pc_extension:
                    continue
                if extension in numpy_extension:
                    continue
                img_file = os.path.join(object_file, image_file) 
                img_dir = os.path.join(img_file, image_file + ".png")                

                for img in os.listdir(img_file):
                    img_name , extension = os.path.splitext(img)
                    if extension in img_extension:
                        continue
                    if extension in numpy_extension:
                        continue
                    txt_file = os.path.join(img_file, img) 
                    txt_dir = os.path.join(txt_file, img + ".txt")                
        ...
        print_log(f'[DATASET] {len(self.file_list)} instances were loaded', logger='TrainShape')
    ...
    def read_point_cloud(self,file_path):
        ...
        if point_cloud.is_empty():
            raise ValueError("La nube de puntos está vacía o no se pudo leer.")

        # Obtener posiciones y colores
        points = np.asarray(point_cloud.points)  # Arreglo de posiciones (N, 3)
        colors = np.asarray(point_cloud.colors)  # Arreglo de colores (N, 3)

        return points, colors
    
    def random_sample_colored(self,n_points, xyz, rgb, num):
        permutation = np.arange(n_points)
        np.random.shuffle(permutation)
        xyz = xyz[permutation[:num]]
        rgb = rgb[permutation[:num]]
        return xyz, rgb

    def get_pc_colored(self, n_points, filepath):
        xyz_data, rgb_data = self.read_point_cloud(filepath)
        xyz_data = xyz_data.astype(np.float32)
        rgb_data = rgb_data.astype(np.float32)
        xyz_data, rgb_data = self.random_sample_colored(n_points, xyz_data, rgb_data, n_points)
        xyz_data = self.pc_norm(xyz_data)
        data = np.concatenate([xyz_data, rgb_data], axis=1)
        data = torch.from_numpy(data).float()

        return data

    def tokenizer_handmade(self, tokenizer, text, max_context_length = 77):
    ...
    def __getitem__(self, idx):
        ...
        data = self.get_pc_colored(self.npoints,sample['pc_dir'])
        ...
        #else:
        #    tokenized_captions.append(self.tokenizer(caption))
        with open(sample["txt_dir"], "r", encoding="utf-8", errors="replace") as f:
            caption = f.read()
            #tokenized_captions.append(self.tokenizer(caption))
            tokenized_captions = self.tokenizer_handmade(self.tokenizer, caption,77)
            

        #tokenized_captions = torch.stack(tokenized_captions)

        
        picked_image_addr = sample["img_dir"]
        ...
        return sample['txt_dir'], sample['img_dir'], tokenized_captions, data, image
...
class ShapeVal(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return sample['txt_dir'], sample['img_dir'], tokenized_captions, data, image, target
    def __len__(self):
        return len(self.file_list)
...
class ShapeTest(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with open(sample["txt_dir"], "r", encoding="utf-8", errors="replace") as f:
            ...
            #tokenized_captions.append(self.tokenizer(caption))
            #arreglo
            #necesita usar batch size 1
            tokenized_captions = self.tokenizer_handmade(self.tokenizer, caption,77)
        #comentar en caso de usar el arreglo
        #tokenized_captions = torch.stack(tokenized_captions)
        ...
        name = sample['pc_dir']
        text = caption
        return sample['txt_dir'], sample['img_dir'], tokenized_captions, data, image, target, text, name
    def __len__(self):
        return len(self.file_list)

@DATASETS.register_module()
class ShapeTrain_Embed(data.Dataset):
    def __init__(self, config):
    ...
    def __getitem__(self, idx):
        ...
        try:
            base, _ = os.path.splitext(sample["txt_dir"])
            txt_tensor_dir = base + '.npy'
            txt_tensor = np.load(txt_tensor_dir)
            txt_tensor = torch.from_numpy(txt_tensor)
        except:
            raise ValueError("text_emb is corrupted: {}".format(txt_tensor_dir))
        try:
            base, _ = os.path.splitext(sample['img_dir'])
            img_tensor_dir = base + '.npy'
            img_tensor = np.load(img_tensor_dir)
            img_tensor = torch.from_numpy(img_tensor)
        except:
            raise ValueError("img_emb is corrupted: {}".format(img_tensor_dir))



        return sample['txt_dir'], sample['img_dir'], txt_tensor, data, img_tensor
...
@DATASETS.register_module()
class ShapeVal_Embed(data.Dataset):
    def __init__(self, config):
    ...
    def __getitem__(self, idx):
        ...
        return sample['txt_dir'], sample['img_dir'], txt_tensor, data, img_tensor, target
...
@DATASETS.register_module()
class ShapeTest_Embed(data.Dataset):
    def __init__(self, config):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            raise ValueError("text is corrupted or not found: {}".format(sample["txt_dir"]))
        ...
        if torch.isnan(txt_tensor).any():
            print("Warning: NaNs in text features")
            print(sample['txt_dir'])
        if torch.isnan(img_tensor).any():
            print("Warning: NaNs in image features")
            print(sample['img_dir'])
        #if self.train_transform != None:
        #        image = self.train_transform(image)
        ...
        name = sample['pc_dir']
        text = caption
        return sample['txt_dir'], sample['img_dir'], txt_tensor, data, img_tensor, target, text, name
...
@DATASETS.register_module()
class Individual_Test(data.Dataset):
    def __init__(self, config):
